...
class ExperienceEvidenceAgent(BaseAgent):
    def __init__(
        ...
        **kwargs
        ...
        if "temperature" not in kwargs:
            kwargs = PRR_DEFAULT_MODELS_CFG["experience_evidence_agent"]
    ...
    # ------------------------------ Calling -------------------------- #
    def prepare_experience_evidence_evaluator_ip(self, job_desc: str, proposal_text: str) -> str:
        formatted = f"""##Job Description:\n{job_desc}\n\n##Proposal:\n{proposal_text}"""
        return formatted
    
    def invoke(self, job_desc: str, proposal_text: str) -> ExperienceEvidenceSchema:
        formatted_input = self.prepare_experience_evidence_evaluator_ip(job_desc, proposal_text)
