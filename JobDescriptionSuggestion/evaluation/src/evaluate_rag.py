...
def retrieve(query: str, embedding_model, collection, n_to_return: int = 10, alpha: float = 0.7):
    """The Retrieval Operation"""

    embedded_query = embedding_model.embed_query(query)
    retrieved_objects = collection.query.hybrid(
        query = query,
...
def rerank(query, cross_encoder, retrieved_objects, n_to_return = 5):
    model_inputs = [[query, obj.properties.get("chunk_text")] for obj in retrieved_objects]

    scores = cross_encoder.predict(model_inputs)

    chunks_with_scores = list(zip(retrieved_objects, scores))
...
# Calc Metrics
def calc_recall_at_k(relevant_chunks_ids, retrieved_chunks_ids):
    total_relevant = len(relevant_chunks_ids)
    n_retrieved_relevant = 0

    for chunk_id in retrieved_chunks_ids:
        if chunk_id in relevant_chunks_ids:
            n_retrieved_relevant += 1

    recall = n_retrieved_relevant / total_relevant
    return recall




def calc_precision_at_k(relevant_chunks_ids, retrieved_chunks_ids):
    total_retrieved = len(retrieved_chunks_ids)
    n_retrieved_relevant = 0
    ...
    precision = n_retrieved_relevant / total_retrieved
    return precision


def calc_mrr(job_topic: str, retrieved_chunks_topics: list[str]):
    """Calc the Mean Reciporcal Rank based on the topic"""
    rank = 0

    for idx, topic in enumerate(retrieved_chunks_topics, start = 1):
        if topic == job_topic:
            rank = 1 / idx
            break
...
def evaluate_retrieval_operation(
        eval_data: list, 
        embedding_model, 
        reranker,
        collection, 
        n_to_retrieve: int = 10,
        n_to_return: int = 5, 
        alpha: float = 0.7
    ):
    ...
    for sample in eval_data:
        ...
        # retrieve & calc recall
        retrieved_objects = retrieve(
            query = query,
            embedding_model = embedding_model,
            collection = collection,
            n_to_return = n_to_retrieve,
            alpha = alpha
        )

        recall_at_k = calc_recall_at_k(
            relevant_chunks_ids = sample["relevant_chunks_ids"],
            retrieved_chunks_ids = [obj.properties.get("chunk_id") for obj in retrieved_objects]
        )
        ...
        precision_at_k = calc_precision_at_k(
            relevant_chunks_ids = sample["relevant_chunks_ids"],
            retrieved_chunks_ids = [obj.properties.get("chunk_id") for obj in reranked]
        )
        ...
        mrr = calc_mrr(
            ...
            retrieved_chunks_topics = [obj.properties.get("chunk_topic") for obj in reranked]
        )
        total_mrr += mrr
